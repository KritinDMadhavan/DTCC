      ...
      const fetchFairnessData = async () => {
        ...
        try {
          ...
          const response = await axios.get(
            `https://block-convey-p1.uc.r.appspot.com/ml/fairness/${id}/${modelId}/${model_version}`,
            {
              headers: {
                    ...
                    ([feature, metrics]: [string, any]) => {
                      ...
                      const combinedData = demographicParityData.map(
                        (item, index) => ({
                          ...
                          equalOpportunity:
                            equalOpportunityData[index]?.value || 0,
                          statisticalParity:
                            statisticalParityData[index]?.value || 0,
                        })
                      );

                      // Get metric summaries for overview cards
                      const dpSummary = getMetricSummary(
                        metrics.demographic_parity
                      );
                      const eoSummary = getMetricSummary(
                        metrics.equal_opportunity
                      );
                      const diSummary = getMetricSummary(
                        metrics.disparate_impact
                      );
                      const spSummary = getMetricSummary(
                        metrics.statistical_parity
                      );
                          ...
                          {combinedData.length > 0 && (
                            <div className="mb-6">
                              ...
                              <p className="text-sm text-gray-600 mt-2">
                                This chart shows how fairness metrics vary
                                across different threshold values. Values closer
                                to 0 generally indicate better fairness.
                              </p>
                            </div>
                          )}
