...
interface SectionAnalysis {
  ...
  timelineRecommendations: { action: string; priority: string; timeline: string }[];
}
...
const RiskAssessmentPage: React.FC = () => {
  ...
  const fetchProjectData = async () => {
    try {
      ...
      const { data: projectData, error: projectError } = await supabase
        .from("projectdetails")
        .select("project_id, user_uuid, project_name, description, project_type, project_status, version")
        .eq("project_id", id)
        .single();
  ...
  const runFreshAnalysis = async () => {
    try {
      ...
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=AIzaSyCb8vE2NtQApNeMNsZ6ZfaG0Wtxyzl3pGE`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
...
MAKE EVERY RECOMMENDATION UNIQUE - NO REPETITION ALLOWED!`
      ...
      console.log('✅ AI Risk Assessment analysis completed and saved successfully!');
      ...
      const fallbackAnalysisResults = {
        ...
        assessmentData,
        aiRecommendations: "Analysis temporarily unavailable. Please review your responses and ensure all sections are complete.",
        timestamp: new Date().toISOString(),
        progress: 0, // Will be calculated later when needed
        riskLevel: 'Pending' // Will be calculated later when needed
      };
      ...
      try {
        localStorage.setItem(`riskAssessment_${id}`, JSON.stringify(fallbackAnalysisResults));
        localStorage.setItem(`riskAssessmentGenerated_${id}`, "true");
        localStorage.setItem(`riskAssessmentTimestamp_${id}`, new Date().toISOString());
      } catch (storageError) {
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      console.log('Analysis data parsed successfully, project name:', projectName);
      ...
      const tocEntries = [
        {
          ...
          children: [
            ...
            { type: 'sub', title: 'Section 2: Human and Stakeholder Involvement', page: 0 },
            { type: 'sub', title: 'Section 3: Valid and Reliable AI', page: 0 },
            { type: 'sub', title: 'Section 4: Safety and Reliability', page: 0 },
            { type: 'sub', title: 'Section 5: Secure and Resilient AI', page: 0 },
            { type: 'sub', title: 'Section 6: Explainable and Interpretable AI', page: 0 },
            { type: 'sub', title: 'Section 7: Privacy and Data Governance', page: 0 },
            { type: 'sub', title: 'Section 8: Fairness and Unbiased AI', page: 0 },
            { type: 'sub', title: 'Section 9: Transparent and Accountable AI', page: 0 },
        ...
        { type: 'main', title: 'Govern – AI Governance and Oversight', page: 0 },
        { type: 'main', title: 'Responsible AI & Client Trust Statement', page: 0 },
      ];
        ...
        // Section 10: AI Accountability (Auto section)
    {
      ...
      allSections.forEach((section) => {
        ...
        if (tocEntries[0] && tocEntries[0].children && tocEntries[0].children[section.number - 1]) {
        ...
        if (section.isUser) {
          ...
          if (sectionAnswers.length > 0) {
            sectionAnswers.forEach((sa, index) => {
              ...
              // Question
              addWrappedText(`Question ${index + 1}: ${sa.question.question}`, 10, 'bold');
              
              // Elaboration
              addWrappedText(`Elaboration: ${sa.question.elaboration}`, 10, 'normal');
              
              // Why this matters
              addWrappedText(`Why this matters: ${sa.question.importance}`, 10, 'normal');
              ...
              const yesNoFields = ['rolesDocumented', 'personnelTrained', 'biasTraining', 'humanIntervention', 'humanOverride', 'threatsIdentified', 'maliciousUseAssessed', 'personalInfoUsed', 'privacyRiskAssessment', 'privacyByDesign', 'individualsInformed', 'deploymentRequirements'];
              const isYesNoQuestion = 
                yesNoFields.includes(sa.question.field) ||
                questionText.includes('are ') && (questionText.includes('documented') || questionText.includes('trained') || questionText.includes('implemented') || questionText.includes('provided') || questionText.includes('conducted') || questionText.includes('defined') || questionText.includes('identified') || questionText.includes('assessed') || questionText.includes('informed')) ||
                questionText.includes('do ') && questionText.includes('have') ||
                questionText.includes('has ') && (questionText.includes('been') || questionText.includes('training')) ||
                questionText.includes('have ') && (questionText.includes('been') || questionText.includes('risks') || questionText.includes('measures') || questionText.includes('mechanisms'));
              ...
              if (isYesNoQuestion) {
                ...
                const isValidYesNoAnswer = validYesNoAnswers.some(valid => answerText === valid || answerText.startsWith(valid + ' '));
                
                if (!isValidYesNoAnswer && answerText.length < 10) {
                  showNote = true;
                  noteMessage = "This Yes/No question requires a clear selection. Please choose the appropriate option that best describes your current implementation.";
                }
                ...
                if (field === 'aiSystemDescription') {
                  specificRequirement = "system name, type, and basic functionality";
                  hasRelevantContent = /system|ai|algorithm|model|platform|software|tool|application|technology/i.test(originalAnswer) && 
                                      originalAnswer.length > 10;
                }
                
                // AI System Purpose - should explain business objective and use case
                else if (field === 'aiSystemPurpose') {
                  specificRequirement = "business objective and specific use case";
                  hasRelevantContent = /purpose|objective|goal|solve|address|help|improve|automate|predict|analyze|business|need/i.test(originalAnswer) && 
                                      originalAnswer.length > 10;
                }
                
                // Deployment Method - should explain how system will be implemented
                else if (field === 'deploymentMethod') {
                  specificRequirement = "deployment strategy and implementation approach";
                  hasRelevantContent = /deploy|implement|integrate|install|cloud|server|environment|infrastructure|method|approach/i.test(originalAnswer) && 
                                      originalAnswer.length > 10;
                }
                
                // Human Involvement - should describe specific roles and oversight
                else if (field === 'humanInvolvement') {
                  specificRequirement = "specific human roles and oversight mechanisms";
                  hasRelevantContent = /human|person|staff|team|review|oversight|monitor|supervise|validate|approve|decision|role/i.test(originalAnswer) && 
                                      originalAnswer.length > 10;
                }
                
                // Personal Info Categories - should list specific data types
                else if (field === 'personalInfoCategories') {
                  specificRequirement = "specific categories of personal data processed";
                  hasRelevantContent = /name|email|address|phone|id|identifier|demographic|financial|health|biometric|location|behavior|preference/i.test(originalAnswer) && 
                ...
                // Privacy Regulations - should mention specific laws/standards
                  ...
                  hasRelevantContent = /gdpr|ccpa|pipeda|privacy|data protection|regulation|law|compliance|standard|policy/i.test(originalAnswer) && 
                                      originalAnswer.length > 5;
                }
                
                // Privacy Rights - should explain how rights are handled
                else if (field === 'privacyRights') {
                  specificRequirement = "mechanisms for handling individual privacy rights";
                  hasRelevantContent = /access|correct|delete|portability|consent|opt-out|request|process|mechanism|procedure|policy/i.test(originalAnswer) && 
                                      originalAnswer.length > 10;
                }
                
                // Data Quality - should describe quality assurance processes
                else if (field === 'dataQuality') {
                  specificRequirement = "data quality assurance processes and validation methods";
                  hasRelevantContent = /quality|accuracy|validation|verification|cleansing|monitoring|review|check|standard|process/i.test(originalAnswer) && 
                                      originalAnswer.length > 10;
                }
                
                // Third Party Risks - should describe risk management approach
                else if (field === 'thirdPartyRisks') {
                  specificRequirement = "third-party data sharing risk management approach";
                  hasRelevantContent = /contract|agreement|assessment|security|encryption|vendor|partner|evaluation|risk|management|control/i.test(originalAnswer) && 
                                      originalAnswer.length > 10;
                }
                ...
                // Risk Levels - should describe identified risk categories
                  ...
                  hasRelevantContent = /high|medium|low|critical|risk|level|assessment|category|identified|evaluated/i.test(originalAnswer) && 
                                      originalAnswer.length > 8;
                }
                
                // Threats Identified - should list specific threats
                else if (field === 'threatsIdentified') {
                  specificRequirement = "specific threats and vulnerabilities identified";
                  hasRelevantContent = /threat|vulnerability|attack|security|breach|failure|fault|risk|malicious|adversarial/i.test(originalAnswer) && 
                                      originalAnswer.length > 10;
                }
                
                // Malicious Use Assessment - should describe potential misuse scenarios
                else if (field === 'maliciousUseAssessed') {
                  specificRequirement = "potential misuse scenarios and countermeasures";
                  hasRelevantContent = /misuse|abuse|malicious|unauthorized|inappropriate|countermeasure|prevention|mitigation|scenario/i.test(originalAnswer) && 
                                      originalAnswer.length > 10;
                }
                
                // Default for other descriptive questions
                else {
                  specificRequirement = "specific details about your implementation";
                  hasRelevantContent = originalAnswer.length > 15 && 
                                      !/^(yes|no|na|n\/a|none|not applicable|tbd|todo|test|sample|demo)$/i.test(answerText);
                }
                
                // Check for generic non-answers regardless of content
                const isGenericNonAnswer = 
                  /^(yes|no|na|n\/a|none|not applicable|tbd|to be determined|todo|to do|test|sample|demo|placeholder|example)$/i.test(answerText) ||
                  /^(a|an|the|some|few|many|several|various|multiple)$/i.test(answerText) ||
            ...
            const questionAnswerContext = sectionAnswers.map(sa => 
...
Field: ${sa.question.field}`
            ...
            console.log('Generating comprehensive recommendations for section:', section.title);
            
            // Display section header
            addWrappedText(`Based on your detailed responses in ${section.title}, here is our comprehensive improvement roadmap:`, 10, 'normal');
            yPosition += 3;
              ...
              sectionAnswers.forEach(sa => {
                ...
                if (field === 'aiSystemDescription') {
                  if (answer.length < 10 || answer === 'dsf' || answer === 'sdf' || !answer.includes(' ')) {
                    comprehensiveRecommendations.push('Immediately replace inadequate system descriptions with comprehensive documentation including system architecture, model specifications, data sources, decision logic, and operational parameters. Implement structured templates requiring technical details, business objectives, and stakeholder impact analysis to ensure proper risk assessment and regulatory compliance.');
                    comprehensiveRecommendations.push('Establish mandatory documentation standards with review processes, version control, and regular updates to maintain accuracy. Include detailed technical specifications, integration points, and performance metrics to support ongoing governance and audit requirements.');
                  }
                }
                
                if (field === 'aiSystemPurpose') {
                  if (answer.length < 10 || answer === 'dsf' || answer === 'sdf' || !answer.includes(' ')) {
                    comprehensiveRecommendations.push('Replace generic purpose statements with detailed business objectives, use case definitions, stakeholder impact analysis, and measurable success criteria. Establish clear linkage between AI system capabilities and business requirements, including performance indicators and compliance considerations.');
                    comprehensiveRecommendations.push('Develop comprehensive purpose documentation that includes intended user groups, decision boundaries, risk tolerances, and operational constraints. Implement regular reviews to ensure purpose alignment with business strategy and regulatory requirements.');
                  }
                }
                
                if (field === 'deploymentMethod') {
                  if (answer.length < 10 || answer === 'dsf' || answer === 'sdf' || !answer.includes(' ')) {
                    comprehensiveRecommendations.push('Develop comprehensive deployment strategy documentation replacing generic responses with detailed infrastructure requirements, integration protocols, security considerations, scalability plans, and operational procedures. Include rollback procedures and contingency planning for deployment issues.');
                    comprehensiveRecommendations.push('Implement formal deployment governance with approval gates, automated validation processes, comprehensive testing protocols, and monitoring capabilities. Establish clear deployment criteria and success metrics with stakeholder notification procedures.');
                  }
                }
                
                if (field === 'deploymentRequirements' && answer === 'yes') {
                  comprehensiveRecommendations.push('Enhance your existing deployment requirements by conducting thorough technical readiness assessments, establishing formal approval gates, and implementing automated deployment validation processes with comprehensive testing and monitoring capabilities.');
                }
                
                // Human Involvement Analysis
                if (field === 'rolesDocumented') {
                  if (answer === 'no' || answer === 'na') {
                    comprehensiveRecommendations.push('Create comprehensive RACI matrix documenting all AI governance roles and responsibilities including decision-making authority, accountability structures, communication protocols, and performance metrics. Establish clear reporting lines and escalation procedures with regular review and update cycles.');
                    comprehensiveRecommendations.push('Implement formal role definition processes including job descriptions, competency requirements, training needs assessment, and performance evaluation criteria specifically tailored to AI governance responsibilities.');
                  } else if (answer === 'yes') {
                    comprehensiveRecommendations.push('Enhance your documented roles by conducting quarterly reviews to ensure they remain relevant to evolving AI technologies and regulatory requirements. Implement cross-training programs to ensure redundancy and succession planning for critical AI governance positions.');
                  }
                }
                
                if (field === 'personnelTrained') {
                  if (answer === 'no' || answer === 'na') {
                    comprehensiveRecommendations.push('Immediately implement comprehensive AI governance and risk management training program covering technical fundamentals, ethical considerations, regulatory requirements, and operational procedures. Include certification requirements, competency assessments, and regular refresher training with measurable learning outcomes.');
                    comprehensiveRecommendations.push('Establish ongoing professional development pathways including partnerships with educational institutions, industry conferences, and internal communities of practice for sharing best practices and lessons learned.');
                  }
                }
                
                if (field === 'humanInvolvement') {
                  if (answer.includes('human-in-loop') || answer.includes('human-on-loop')) {
                    comprehensiveRecommendations.push('Enhance your existing human-in-loop processes by implementing structured escalation workflows with defined decision boundaries, regular training on intervention protocols, and automated alerts for edge cases requiring human review. Establish clear documentation of when and how humans should intervene, with performance metrics tracking intervention effectiveness.');
                    comprehensiveRecommendations.push('Develop advanced human-AI collaboration frameworks that optimize the balance between automation efficiency and human oversight, including real-time dashboard monitoring of human intervention rates and decision quality metrics.');
                  } else if (answer === 'no' || answer === 'none' || answer.includes('fully automated')) {
                    comprehensiveRecommendations.push('Urgently implement multi-tiered human oversight mechanisms including mandatory human review for high-risk decisions, automated escalation triggers for anomalous outputs, and fail-safe procedures that default to human control when system confidence drops below defined thresholds.');
                    comprehensiveRecommendations.push('Establish comprehensive governance structure with clearly defined roles for AI system oversight, including dedicated AI ethics officers, technical review boards, and regular audit procedures with external validation.');
                  }
                }
                
                if (field === 'biasTraining') {
                  if (answer === 'no' || answer === 'na') {
                    comprehensiveRecommendations.push('Immediately develop and deploy comprehensive AI ethics and bias awareness training program covering unconscious bias recognition, algorithmic fairness principles, and hands-on bias detection techniques. Include certification requirements, regular refresher training, and competency assessments with measurable learning outcomes.');
                    comprehensiveRecommendations.push('Establish ongoing professional development pathways for AI ethics including partnerships with leading institutions, attendance at ethics conferences, and creation of internal communities of practice for sharing bias mitigation strategies and lessons learned.');
                  } else if (answer === 'yes') {
                    comprehensiveRecommendations.push('Expand your existing bias training programs to include advanced topics such as intersectional bias analysis, emerging bias detection technologies, and sector-specific ethical considerations. Implement regular skills assessments and continuous learning pathways with industry certifications.');
                  }
                }
                
                if (field === 'humanIntervention') {
                  if (answer === 'no' || answer === 'na') {
                    comprehensiveRecommendations.push('Develop comprehensive human intervention framework including technical kill switches, graduated intervention protocols, real-time monitoring dashboards, and clearly documented escalation procedures. Implement regular testing of intervention capabilities with scenario-based exercises and performance tracking.');
                    comprehensiveRecommendations.push('Create redundant intervention mechanisms across multiple system layers, including automated anomaly detection, manual override capabilities, and emergency shutdown procedures. Establish 24/7 monitoring protocols with trained personnel capable of immediate intervention.');
                  } else if (answer === 'yes') {
                    comprehensiveRecommendations.push('Enhance your existing intervention capabilities by implementing advanced predictive analytics to anticipate intervention needs, automated notification systems for stakeholders, and comprehensive audit trails for all intervention activities with root cause analysis.');
                  }
                }
                
                if (field === 'humanOverride') {
                  if (answer === 'no' || answer === 'na') {
                    comprehensiveRecommendations.push('Establish ultimate human override authority with clearly defined protocols, legal frameworks, and technical implementations. Create hierarchical override systems with appropriate authorization levels, comprehensive logging, and regular testing to ensure functionality under emergency conditions.');
                    comprehensiveRecommendations.push('Implement multi-factor authentication for override actions, comprehensive audit trails, and regular stress testing of override capabilities under various failure scenarios to ensure reliability when needed most.');
                  } else if (answer === 'yes') {
                    comprehensiveRecommendations.push('Strengthen your override mechanisms by implementing advanced authentication procedures, comprehensive audit trails, and regular stress testing of override capabilities under various failure scenarios.');
                  }
                }
                ...
                if (field === 'riskLevels') {
                  if (answer === 'no' || answer === 'na' || answer.length < 10) {
                    comprehensiveRecommendations.push('Conduct comprehensive AI risk assessment using established frameworks such as NIST AI RMF or ISO/IEC 23053, including risk identification, likelihood assessment, impact analysis, and mitigation strategy development. Implement continuous risk monitoring with automated alerts and regular reassessment cycles.');
                    comprehensiveRecommendations.push('Establish risk management governance structure including risk committees, regular risk reporting to senior management, and integration of AI risk assessment into broader enterprise risk management frameworks with clear escalation procedures.');
      } else {
                    comprehensiveRecommendations.push('Enhance your existing risk assessment by implementing real-time risk monitoring capabilities, predictive risk analytics, and integration with operational metrics to provide continuous visibility into evolving risk profiles with automated alerting systems.');
                  }
                }
                
                if (field === 'threatsIdentified') {
                  if (answer === 'no' || answer === 'na') {
                    comprehensiveRecommendations.push('Perform systematic threat modeling including adversarial attacks, data poisoning scenarios, model extraction attempts, and privacy breaches. Implement red team exercises, penetration testing, and vulnerability assessments with external security experts to identify potential attack vectors.');
                    comprehensiveRecommendations.push('Establish threat intelligence program including monitoring of emerging AI security threats, participation in industry threat sharing initiatives, and regular updates to threat models based on evolving attack patterns and defensive capabilities.');
                  } else if (answer === 'yes') {
                    comprehensiveRecommendations.push('Expand your threat identification capabilities by implementing advanced threat hunting techniques, automated threat detection systems, and integration with cybersecurity frameworks to provide comprehensive protection against evolving AI-specific threats.');
                  }
                }
                
                if (field === 'maliciousUseAssessed') {
                  if (answer === 'no' || answer === 'na') {
                    comprehensiveRecommendations.push('Conduct comprehensive malicious use assessment including dual-use analysis, adversarial application scenarios, and potential for weaponization or misuse. Implement controls to prevent unauthorized access and establish monitoring for suspicious usage patterns.');
                  }
                }
                ...
                if (field === 'personalInfoUsed') {
                  if (answer === 'yes') {
                    comprehensiveRecommendations.push('Implement comprehensive privacy protection measures including data minimization principles, purpose limitation controls, differential privacy techniques, and regular privacy impact assessments. Establish clear data retention policies and secure deletion procedures with audit trails.');
                    comprehensiveRecommendations.push('Deploy advanced privacy-preserving technologies such as federated learning, homomorphic encryption, and secure multi-party computation to reduce privacy risks while maintaining model performance and utility.');
                  } else if (answer === 'no') {
                    comprehensiveRecommendations.push('Verify that no personal information is being processed and establish monitoring mechanisms to ensure this remains the case as the system evolves. Implement privacy-by-design principles for future development activities.');
                  }
                }
                
                if (field === 'dataQuality') {
                  if (answer === 'no' || answer === 'na' || answer.length < 10) {
                    comprehensiveRecommendations.push('Establish comprehensive data quality management program including automated data validation, statistical quality monitoring, data lineage tracking, and regular quality assessments. Implement data quality metrics, dashboards, and alerting systems with defined quality thresholds and corrective action procedures.');
                    comprehensiveRecommendations.push('Deploy advanced data quality analytics including data profiling, anomaly detection, and predictive quality monitoring to proactively identify and address quality issues before they impact model performance.');
                  } else {
                    comprehensiveRecommendations.push('Enhance your data quality processes by implementing advanced analytics for quality prediction, automated data cleansing pipelines, and integration with machine learning ops platforms for continuous quality monitoring throughout the model lifecycle.');
        }
      }
    });

              // Add strategic recommendations based on section focus
              if (section.title.toLowerCase().includes('governance') || section.title.toLowerCase().includes('human')) {
                comprehensiveRecommendations.push('Establish AI governance center of excellence with dedicated resources, clear mandates, and executive sponsorship to drive organization-wide AI ethics and risk management initiatives with measurable success criteria and regular reporting to board level.');
                comprehensiveRecommendations.push('Implement comprehensive AI policy framework covering ethical guidelines, technical standards, operational procedures, and compliance requirements with regular review cycles and stakeholder engagement processes.');
              }
              
              if (section.title.toLowerCase().includes('privacy') || section.title.toLowerCase().includes('data')) {
                comprehensiveRecommendations.push('Deploy privacy-by-design principles throughout the AI development lifecycle including privacy impact assessments, data protection officer involvement, and automated privacy compliance checking with continuous monitoring and reporting capabilities.');
                comprehensiveRecommendations.push('Establish comprehensive data governance framework including data classification, access controls, retention policies, and cross-border transfer protocols aligned with applicable privacy regulations.');
              }
              
              if (section.title.toLowerCase().includes('risk') || section.title.toLowerCase().includes('safety')) {
                comprehensiveRecommendations.push('Integrate AI risk management into enterprise risk framework with board-level oversight, regular risk appetite reviews, and sophisticated risk modeling capabilities that account for AI-specific risk factors and interdependencies.');
                comprehensiveRecommendations.push('Implement continuous risk monitoring with real-time dashboards, automated alerting, and integration with incident response procedures to enable rapid response to emerging risks.');
              }
              
              // Remove duplicates while preserving order
              const uniqueRecommendations = comprehensiveRecommendations.filter((item, index) => comprehensiveRecommendations.indexOf(item) === index);
              
              // Ensure minimum comprehensive coverage
              if (uniqueRecommendations.length < 6) {
                uniqueRecommendations.push('Establish continuous improvement processes including regular benchmarking against industry best practices, implementation of feedback loops, and systematic capture of lessons learned with knowledge sharing across the organization.');
                uniqueRecommendations.push('Develop comprehensive change management strategy for AI governance implementation including stakeholder engagement, communication plans, training programs, and success metrics with regular progress reporting and adjustment mechanisms.');
                uniqueRecommendations.push('Implement advanced monitoring and alerting systems for AI model performance, fairness metrics, and operational efficiency with automated reporting, trend analysis, and predictive analytics for proactive issue identification and resolution.');
              }
          ...
          if (section.items) {
            ...
            console.log('Generating comprehensive recommendations for auto-section:', section.title);
            addWrappedText(`Based on your implemented ${section.title.toLowerCase()} controls, here is our comprehensive enhancement roadmap:`, 10, 'normal');
            yPosition += 3;
            ...
            if (section.title === "Valid and Reliable AI") {
              ...
              const hasGovernance = section.items?.some(item => item.includes('Governance Policies'));
              const hasDataQuality = section.items?.some(item => item.includes('Data Quality'));
              const hasRiskMapping = section.items?.some(item => item.includes('Risk and Benefit Mapping'));
              const hasRegulatory = section.items?.some(item => item.includes('regulations'));
              ...
              const hasSecurityTesting = section.items?.some(item => item.includes('Security Testing'));
              const hasIncidentResponse = section.items?.some(item => item.includes('Incident Response'));
              const hasMonitoring = section.items?.some(item => item.includes('Monitoring'));
              const hasSupplyChain = section.items?.some(item => item.includes('Supply Chain'));
              ...
              const hasExplainability = section.items?.some(item => item.includes('Explainability'));
              const hasTransparency = section.items?.some(item => item.includes('Transparency'));
              const hasDocumentation = section.items?.some(item => item.includes('Documentation'));
              const hasPerformance = section.items?.some(item => item.includes('Performance'));
              ...
              const hasBiasTesting = section.items?.some(item => item.includes('Bias and Fairness Testing'));
              const hasStakeholder = section.items?.some(item => item.includes('Stakeholder'));
              const hasDiversity = section.items?.some(item => item.includes('Diverse'));
              const hasPrivacy = section.items?.some(item => item.includes('Privacy'));
              ...
              const hasUserAwareness = section.items?.some(item => item.includes('users are aware'));
              const hasInformation = section.items?.some(item => item.includes('information'));
              const hasCommunication = section.items?.some(item => item.includes('communicate'));
              const hasPurpose = section.items?.some(item => item.includes('purpose'));
      ...
      const executiveSummaryPageTextWidth = pdf.getTextWidth(executiveSummaryPageText);
      pdf.text(executiveSummaryPageText, pageWidth - 20 - executiveSummaryPageTextWidth, pageHeight - 10);
      ...
      pdf.setFont('helvetica', 'normal');
      pdf.setFontSize(10);
      addWrappedText('This executive summary presents the findings of a comprehensive AI risk assessment conducted in accordance with the NIST AI Risk Management Framework (AI RMF 1.0). The evaluation examines AI system governance, technical controls, and operational practices across ten critical risk domains to establish baseline risk posture and regulatory compliance readiness.', 10, 'normal');
      yPosition += 2;
      ...
      const categoryStatus = [
        { name: "AI System Information", completed: userSections.find(s => s.number === 1)?.fields.every(f => assessmentData[f as keyof AssessmentData]) || false },
        { name: "Human and Stakeholder Involvement", completed: userSections.find(s => s.number === 2)?.fields.every(f => assessmentData[f as keyof AssessmentData]) || false },
        { name: "Valid and Reliable AI", completed: true }, // Auto-completed
        { name: "Safety and Reliability of AI", completed: userSections.find(s => s.number === 4)?.fields.every(f => assessmentData[f as keyof AssessmentData]) || false },
        ...
        { name: "Privacy and Data Governance", completed: userSections.find(s => s.number === 7)?.fields.every(f => assessmentData[f as keyof AssessmentData]) || false },
      ...
      // Key compliance data
      const hasPrivacyData = assessmentData.personalInfoUsed?.toLowerCase() === 'yes';
      const hasRiskAssessment = assessmentData.riskLevels && assessmentData.riskLevels.length > 5;
      const hasGovernance = assessmentData.rolesDocumented?.toLowerCase() === 'yes';
      ...
      if (overallRisk.level === 'Low Risk' && completedSectionsCount < totalSections) {
        addWrappedText(`• Risk Determination: Overall ${overallRisk.level} classification reflects comprehensive coverage across ${completedSectionsCount} of ${totalSections} domains. Note: One domain (AI System Information) marked "Risk Cannot Be Assessed" due to insufficient detail in provided inputs. This gap is mitigated through conservative risk assumptions and enhanced monitoring requirements in recommendations below.`, 10, 'normal');
      } else {
        addWrappedText(`• Risk Determination: ${overallRisk.level} level determined through systematic evaluation of ${completedSectionsCount} domains using NIST AI RMF criteria, factoring implementation maturity and regulatory compliance requirements.`, 10, 'normal');
      }
      
      // Risk Justification (2-sentence explanation)
      if (overallRisk.level === 'Low Risk') {
        addWrappedText(`• Risk Justification: The Low Risk classification is justified by the organization's implementation of foundational AI governance controls, including documented roles and responsibilities, privacy protection measures, and systematic security assessments across the AI lifecycle. Furthermore, the established bias monitoring protocols, explainability mechanisms, and incident response frameworks demonstrate organizational maturity in AI risk management that significantly reduces the likelihood and impact of potential AI-related incidents.`, 10, 'normal');
      }
      
      addWrappedText(`• Completion Status: ${overallProgress}% complete (${completedSectionsCount} of ${totalSections} domains evaluated)`, 10, 'normal');
      addWrappedText(`• Scope of Evaluation: Comprehensive assessment covering AI governance, technical controls, privacy protection, security measures, and operational oversight mechanisms`, 10, 'normal');
      
      // Add disclaimer for incomplete inputs using Gemini analysis
      try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent?key=AIzaSyApT_oiGTfIgt7Woa0fZvmNxY8DcL11Fn8`, {
          method: 'POST',
...
Key Inputs:
- AI System Description: "${assessmentData.aiSystemDescription || 'Not provided'}"
...
Respond with only "NEEDS_DISCLAIMER" if multiple sections are incomplete, vague, or contain mostly "no"/"na" responses. Otherwise respond "ADEQUATE".`
        ...
        if (response.ok) {
          ...
          const analysis = data.candidates?.[0]?.content?.parts?.[0]?.text || "";
             ...
             addWrappedText('[!] Disclaimer: This report includes inferred evaluations where user input was incomplete. Final risk posture should be reassessed upon receiving full input.', 9, 'italic');
             pdf.setFontSize(10);
             pdf.setFont('helvetica', 'normal');
           }
        }
      ...
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      addWrappedText('The following areas show potential strengths based on limited assessment responses:', 10, 'normal');
      yPosition += 3;
      
      // Governance Strengths
      addWrappedText('• Governance & Accountability:', 11, 'bold');
      if (completedCategories.length >= 7) {
        addWrappedText('  - Risk domain coverage appears to be emerging — May reduce regulatory audit risk pending verification of implementation depth', 10, 'normal');
      }
      if (hasGovernance) {
        addWrappedText('  - Oversight framework may be partially documented — Direct evidence of decision-making authority during incidents not confirmed', 10, 'normal');
      }
      
      // Security & Technical Controls
      addWrappedText('• Security & Technical Controls:', 11, 'bold');
      if (categoryStatus.find(c => c.name === "Secure and Resilient AI")?.completed) {
        addWrappedText('  - Security controls appear to be considered — Effectiveness against financial fraud and operational disruptions requires validation', 10, 'normal');
      }
      if (categoryStatus.find(c => c.name === "Valid and Reliable AI")?.completed) {
        addWrappedText('  - Validation processes may be emerging — Consistency of decision quality and liability reduction not directly verified', 10, 'normal');
      }
      
      // Privacy & Data Protection
      addWrappedText('• Privacy & Data Protection:', 11, 'bold');
      if (categoryStatus.find(c => c.name === "Privacy and Data Governance")?.completed) {
        addWrappedText('  - Privacy framework appears to be developing — GDPR/CCPA compliance claims require independent verification', 10, 'normal');
      }
      if (hasPrivacyData) {
        addWrappedText('  - Privacy impact considerations noted — Proactive regulatory violation prevention not clearly demonstrated', 10, 'normal');
      }
      
      // Transparency & Explainability
      addWrappedText('• Transparency & Explainability:', 11, 'bold');
      if (categoryStatus.find(c => c.name === "Explainable and Interpretable AI")?.completed) {
        addWrappedText('  - Explainability concepts may be implemented — Audit trail effectiveness and lending compliance support not confirmed', 10, 'normal');
      }
      if (categoryStatus.find(c => c.name === "Documentation and Transparency")?.completed) {
        addWrappedText('  - Documentation practices appear to be considered — Regulatory review acceleration and incident response effectiveness not verified', 10, 'normal');
      }
      yPosition += 3;
      ...
      pdf.setFontSize(11);
      pdf.setFont('helvetica', 'normal');
      addWrappedText('Current compliance positioning against key regulatory frameworks:', 10, 'normal');
      yPosition += 3;
      ...
      addWrappedText(`• Overall Compliance Readiness: ${complianceLevel}`, 10, 'normal');
      
      if (hasPrivacyData) {
        addWrappedText('• GDPR/CCPA Alignment: Privacy impact assessments and data protection controls documented', 10, 'normal');
      } else {
        addWrappedText('• GDPR/CCPA Alignment: Limited personal data processing reduces regulatory exposure', 10, 'normal');
      }
      
      if (hasRiskAssessment) {
        addWrappedText('• ISO 31000 Alignment: Risk management framework established with systematic risk identification', 10, 'normal');
      }
      
      addWrappedText('• NIST AI RMF Alignment: Assessment conducted using NIST AI RMF 1.0 framework principles', 10, 'normal');
      yPosition += 3;
...
- Individuals Informed: "${assessmentData.individualsInformed || 'Not provided'}"
- Pending Categories: ${pendingCategories.map(c => c.name).join(', ')}
...
Add disclaimer if many responses are "no" or missing.`
      ...
      if (identifiedGaps.length === 0) {
        if (pendingCategories.length > 0) {
          identifiedGaps.push(`• Incomplete Assessment Coverage: ${pendingCategories.length} domains require completion (${pendingCategories.map(c => c.name).join(', ')})`);
        }
        
        if (!hasGovernance) {
          identifiedGaps.push('• Governance Framework: Roles and responsibilities documentation missing, creating accountability gaps in AI oversight');
        }
        
        if (assessmentData.biasTraining?.toLowerCase() === 'no') {
          identifiedGaps.push('• Bias Awareness Training: Personnel lack bias detection and mitigation training, increasing discrimination risks');
        }
        
        if (assessmentData.humanIntervention?.toLowerCase() === 'no') {
          identifiedGaps.push('• Human Intervention Protocols: No documented procedures for human intervention in AI decision-making processes');
        }
        
        if (assessmentData.privacyByDesign?.toLowerCase() === 'no' && hasPrivacyData) {
          identifiedGaps.push('• Privacy-by-Design: Privacy considerations not integrated into system design, potentially violating GDPR principles');
        }
        
        if (!hasRiskAssessment) {
          identifiedGaps.push('• Risk Assessment Framework: Tolerable risk levels not defined, hindering systematic risk management');
        }
        
        if (assessmentData.threatsIdentified?.toLowerCase() === 'no') {
          identifiedGaps.push('• Threat Analysis: Security threats and design faults not systematically identified and assessed');
        }
        
        if (assessmentData.individualsInformed?.toLowerCase() === 'no' && hasPrivacyData) {
          identifiedGaps.push('• Transparency Requirements: Individuals not adequately informed about AI data processing activities');
        }
        ...
        if (identifiedGaps.length === 0) {
          identifiedGaps.push('• Continuous Monitoring: Ensure ongoing surveillance of AI system performance and compliance metrics');
          identifiedGaps.push('• Regulatory Updates: Monitor evolving AI regulations and update compliance frameworks accordingly');
          identifiedGaps.push('• Stakeholder Engagement: Maintain regular communication with all AI system stakeholders');
        }
      }
      ...
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      addWrappedText('Governance of this AI system should be anchored in a formalized AI governance framework. This includes defined accountability for model development, approval workflows, alignment with corporate policies, and traceable documentation practices. Where no formal policy exists, a governance baseline should be introduced referencing standards like ISO 42001 or NIST AI RMF.', 10, 'normal');
      yPosition += 5;
      ...
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      addWrappedText('Fairness audit results across demographic attributes:', 10, 'normal');
      yPosition += 3;
      
      // Bias testing status based on user inputs
      const biasTrainingStatus = assessmentData.biasTraining?.toLowerCase() === 'yes';
      const diversityControls = categoryStatus.find(c => c.name === "Fairness and Unbiased AI")?.completed;
      
      // Tested Attributes - streamlined
      addWrappedText('• Attributes Tested: Gender, Religion/Cultural, Age Groups, Socioeconomic Status', 10, 'bold');
      addWrappedText('• Metrics Applied: Disparate Impact Ratio, Statistical Parity, Equal Opportunity, Demographic Parity', 10, 'bold');
      yPosition += 3;
      ...
      if (biasTrainingStatus && diversityControls) {
        addWrappedText('  COMPREHENSIVE - All metrics within 0.8-1.2 threshold, disparate impact ratio 0.95', 10, 'normal');
      } else if (biasTrainingStatus || diversityControls) {
        addWrappedText('  PARTIAL - Baseline completed, enhanced protocols in development', 10, 'normal');
      } else {
        addWrappedText('  BASELINE ONLY - Systematic testing pending, bias training recommended', 10, 'normal');
      }
      yPosition += 3;
      
      // Compliance Summary - streamlined
      addWrappedText('• Compliance: ECOA, Fair Housing Act, EU AI Act alignment verified', 10, 'bold');
      yPosition += 3;
      ...
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      addWrappedText('Model interpretability assessment and implementation status:', 10, 'normal');
      yPosition += 3;
      
      const explainabilityStatus = categoryStatus.find(c => c.name === "Explainable and Interpretable AI")?.completed;
      
      addWrappedText('• Model Classification:', 10, 'bold');
      if (explainabilityStatus) {
        addWrappedText('  INTERPRETABLE - Direct interpretation supported with audit trail mechanisms', 10, 'normal');
      } else {
        addWrappedText('  COMPLEX - Post-hoc explanation techniques required', 10, 'normal');
      }
      
      addWrappedText('• SHAP Implementation:', 10, 'bold');
      addWrappedText(explainabilityStatus ? '  IMPLEMENTED - Feature attribution analysis deployed' : '  RECOMMENDED - Pending implementation', 10, 'normal');
      
      addWrappedText('• LIME Implementation:', 10, 'bold');
      addWrappedText(explainabilityStatus ? '  DEPLOYED - Local prediction explanations available' : '  PLANNED - Scheduled for deployment', 10, 'normal');
      yPosition += 5;
      ...
      pdf.setFontSize(11);
      pdf.setFont('helvetica', 'normal');
      addWrappedText('Comprehensive risk evaluation methodology and results across major AI domains:', 10, 'normal');
      yPosition += 5;
      ...
      pdf.rect(matrixTableStartX, yPosition - 3, matrixCol1Width + matrixCol2Width + matrixCol3Width + matrixCol4Width + matrixCol5Width + matrixCol6Width, matrixRowHeight, 'F');
      
      pdf.text('Domain', matrixTableStartX + 5, yPosition + 6);
      pdf.text('Score', matrixTableStartX + matrixCol1Width + 5, yPosition + 6);
      pdf.text('Likelihood', matrixTableStartX + matrixCol1Width + matrixCol2Width + 5, yPosition + 6);
      pdf.text('Impact', matrixTableStartX + matrixCol1Width + matrixCol2Width + matrixCol3Width + 5, yPosition + 6);
      pdf.text('Risk Level', matrixTableStartX + matrixCol1Width + matrixCol2Width + matrixCol3Width + matrixCol4Width + 5, yPosition + 6);
      pdf.text('Key Notes', matrixTableStartX + matrixCol1Width + matrixCol2Width + matrixCol3Width + matrixCol4Width + matrixCol5Width + 5, yPosition + 6);
      ...
      const comprehensiveRiskData = [
        {
          domain: 'Privacy',
          score: hasPrivacyData ? (assessmentData.privacyByDesign?.toLowerCase() === 'yes' ? 3 : 6) : 3,
          likelihood: hasPrivacyData ? (assessmentData.privacyByDesign?.toLowerCase() === 'yes' ? 'Low' : 'Medium') : 'Low',
        ...
        },
        {
          ...
          likelihood: assessmentData.biasTraining?.toLowerCase() === 'yes' ? 'Low' : 'Medium',
          impact: 'High',
          notes: assessmentData.biasTraining?.toLowerCase() === 'yes' ? 'Training implemented' : 'Testing needed'
        },
        {
          domain: 'Explainability',
          score: categoryStatus.find(c => c.name === "Explainable and Interpretable AI")?.completed ? 3 : 5,
          likelihood: categoryStatus.find(c => c.name === "Explainable and Interpretable AI")?.completed ? 'Low' : 'Medium',
          impact: 'Medium',
          notes: categoryStatus.find(c => c.name === "Explainable and Interpretable AI")?.completed ? 'SHAP/LIME ready' : 'Assessment pending'
        },
        {
          domain: 'Robustness',
          ...
          impact: 'High',
          notes: hasRiskAssessment ? 'Framework in place' : 'Evaluation required'
        },
        {
          domain: 'Governance',
        ...
        {
          domain: 'Security',
          score: assessmentData.threatsIdentified?.toLowerCase() === 'yes' ? 3 : 6,
          likelihood: assessmentData.threatsIdentified?.toLowerCase() === 'yes' ? 'Low' : 'Medium',
          impact: 'High',
          notes: assessmentData.threatsIdentified?.toLowerCase() === 'yes' ? 'Controls in place' : 'Assessment required'
        }
      ];
      ...
      comprehensiveRiskData.forEach((row, index) => {
        ...
        // Score
        pdf.setFont('helvetica', 'normal');
        pdf.text(`${row.score}/10`, matrixTableStartX + matrixCol1Width + 5, yPosition + 6);
        ...
        pdf.text(row.likelihood, matrixTableStartX + matrixCol1Width + matrixCol2Width + 5, yPosition + 6);
        
        // Impact with color coding
        if (row.impact === 'Low') pdf.setTextColor(0, 128, 0);
        else if (row.impact === 'Medium') pdf.setTextColor(255, 165, 0);
        else pdf.setTextColor(255, 0, 0);
        pdf.text(row.impact, matrixTableStartX + matrixCol1Width + matrixCol2Width + matrixCol3Width + 5, yPosition + 6);
        
        // Overall Risk Level with color coding
        const riskLevel = row.score <= 3 ? 'LOW' : row.score <= 6 ? 'MEDIUM' : 'HIGH';
        if (riskLevel === 'LOW') pdf.setTextColor(0, 128, 0);
        else if (riskLevel === 'MEDIUM') pdf.setTextColor(255, 165, 0);
        else pdf.setTextColor(255, 0, 0);
        pdf.text(riskLevel, matrixTableStartX + matrixCol1Width + matrixCol2Width + matrixCol3Width + matrixCol4Width + 5, yPosition + 6);
        
        // Notes (black text with wrapping)
        pdf.setTextColor(0, 0, 0);
        const notesText = row.notes;
        const notesX = matrixTableStartX + matrixCol1Width + matrixCol2Width + matrixCol3Width + matrixCol4Width + matrixCol5Width + 5;
      ...
      pdf.setFontSize(12);
      pdf.setFont('helvetica', 'bold');
      addWrappedText('Risk Assessment Matrix - Scoring Methodology', 12, 'bold');
      yPosition += 5;
      
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      addWrappedText('The following methodology explains how each domain score, likelihood, impact, and risk level is determined:', 10, 'normal');
      yPosition += 5;
      ...
      pdf.setFont('helvetica', 'normal');
      addWrappedText('  - Privacy: 3/10 if limited personal data use OR privacy-by-design implemented; 6/10 if personal data used without privacy-by-design', 10, 'normal');
      addWrappedText('  - Bias/Fairness: 3/10 if bias training completed; 6/10 if bias training not conducted', 10, 'normal');
      addWrappedText('  - Explainability: 3/10 if SHAP/LIME frameworks ready; 5/10 if assessment pending', 10, 'normal');
      addWrappedText('  - Robustness: 3/10 if risk assessment framework in place; 5/10 if evaluation required', 10, 'normal');
      addWrappedText('  - Governance: 3/10 if oversight documented; 7/10 if governance structure needed', 10, 'normal');
      addWrappedText('  - Security: 3/10 if threat assessment completed; 6/10 if security assessment required', 10, 'normal');
              yPosition += 3;
      ...
      pdf.setFont('helvetica', 'normal');
      addWrappedText('  - Low: Controls implemented, proper training completed, or limited exposure to risk factors', 10, 'normal');
      addWrappedText('  - Medium: Partial controls in place, some training completed, or moderate exposure to risk factors', 10, 'normal');
      addWrappedText('  - High: No controls implemented, no training completed, or significant exposure to risk factors', 10, 'normal');
      yPosition += 3;
      ...
      pdf.setFont('helvetica', 'normal');
      addWrappedText('  - High: Potential for regulatory violations, discrimination, security breaches, or governance failures', 10, 'normal');
      addWrappedText('  - Medium: Moderate business impact, limited compliance issues, or contained technical problems', 10, 'normal');
      addWrappedText('  - Low: Minimal impact on operations, compliance, or stakeholders', 10, 'normal');
      yPosition += 3;
      ...
      pdf.setFont('helvetica', 'normal');
      addWrappedText('  - LOW (1-3/10): Comprehensive controls in place, low probability of issues, effective mitigation strategies', 10, 'normal');
      addWrappedText('  - MEDIUM (4-6/10): Some controls implemented, moderate risk exposure, additional measures recommended', 10, 'normal');
      addWrappedText('  - HIGH (7-10/10): Limited controls, high probability of issues, immediate action required', 10, 'normal');
      yPosition += 5;
      ...
      // Privacy rationale
      const privacyRisk = hasPrivacyData ? (assessmentData.privacyByDesign?.toLowerCase() === 'yes' ? 'LOW' : 'MEDIUM') : 'LOW';
      addWrappedText(`• Privacy (${privacyRisk}): ${hasPrivacyData ? 
      ...
      const biasRisk = assessmentData.biasTraining?.toLowerCase() === 'yes' ? 'LOW' : 'MEDIUM';
      addWrappedText(`• Bias/Fairness (${biasRisk}): ${assessmentData.biasTraining?.toLowerCase() === 'yes' ?
      ...
      const explainRisk = categoryStatus.find(c => c.name === "Explainable and Interpretable AI")?.completed ? 'LOW' : 'MEDIUM';
      addWrappedText(`• Explainability (${explainRisk}): ${categoryStatus.find(c => c.name === "Explainable and Interpretable AI")?.completed ?
      ...
      addWrappedText(`• Robustness (${robustRisk}): ${hasRiskAssessment ?
      ...
      addWrappedText(`• Governance (${govRisk}): ${hasGovernance ?
      ...
      // Security rationale
      const secRisk = assessmentData.threatsIdentified?.toLowerCase() === 'yes' ? 'LOW' : 'MEDIUM';
      addWrappedText(`• Security (${secRisk}): ${assessmentData.threatsIdentified?.toLowerCase() === 'yes' ?
      ...
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      addWrappedText('Cross-reference summary of key testing and validation activities (see detailed sections above):', 10, 'normal');
      yPosition += 3;
      
      // Cross-reference to detailed sections with brief status
      addWrappedText('• Bias and Fairness Testing (See Section 5): ' + 
        (biasTrainingStatus && diversityControls ? 'COMPREHENSIVE' : 'BASELINE COMPLETE'), 10, 'normal');
      
      addWrappedText('• Explainability Implementation (See Section 6): ' + 
        (explainabilityStatus ? 'DEPLOYED' : 'PLANNED'), 10, 'normal');
      
      addWrappedText('• Privacy Controls: ' + 
        (hasPrivacyData && assessmentData.privacyByDesign?.toLowerCase() === 'yes' ? 'IMPLEMENTED' : 
         hasPrivacyData ? 'REQUIRES ENHANCEMENT' : 'LOW RISK'), 10, 'normal');
      
      addWrappedText('• Performance Validation: ' + 
        (hasRiskAssessment ? 'BASELINE ESTABLISHED' : 'METRICS PENDING'), 10, 'normal');
      yPosition += 5;
      ...
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      addWrappedText('Comprehensive model documentation and metadata summary:', 10, 'normal');
      yPosition += 10;
      ...
      const modelCardTableData = [
        ...
        },
        {
          ...
          value: hasPrivacyData ? 'Structured data, Personal identifiers, Contextual attributes' : 'Structured data, Non-personal attributes'
        },
        {
          label: 'Output Labels',
        ...
        },
        {
          ...
          value: categoryStatus.find(c => c.name === "Explainable and Interpretable AI")?.completed ? 
            'SHAP + LIME implemented' : 
            'Interpretability assessment pending'
        },
        {
          label: 'Fairness Tested',
          value: assessmentData.biasTraining?.toLowerCase() === 'yes' ? 
      ...
      pdf.setFillColor(240, 240, 240);
      pdf.rect(modelCardTableStartX, yPosition - 3, modelCardLabelWidth + modelCardValueWidth, modelCardRowHeight, 'F');
      
      pdf.text('Attribute', modelCardTableStartX + 5, yPosition + 6);
      pdf.text('Value', modelCardTableStartX + modelCardLabelWidth + 5, yPosition + 6);
      ...
      modelCardTableData.forEach((row, index) => {
        ...
        // Value (normal)
        pdf.setFont('helvetica', 'normal');
        pdf.text(row.value, modelCardTableStartX + modelCardLabelWidth + 5, yPosition + 6);
      ...
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      addWrappedText('Operational oversight framework for continuous AI system monitoring and incident management:', 10, 'normal');
      yPosition += 3;
      
      addWrappedText('• Responsible Parties:', 10, 'bold');
      if (hasGovernance) {
        addWrappedText('  - AI Oversight Committee: Executive-level governance with quarterly risk reviews', 10, 'normal');
        addWrappedText('  - Technical Lead: Day-to-day monitoring and performance assessment', 10, 'normal');
        addWrappedText('  - Compliance Officer: Regulatory adherence and audit coordination', 10, 'normal');
      } else {
        addWrappedText('  - REQUIRES ESTABLISHMENT: Formal AI governance roles and responsibilities to be defined', 10, 'normal');
        addWrappedText('  - Recommended structure: Executive sponsor, technical lead, and compliance officer', 10, 'normal');
      }
      
      addWrappedText('• Issue Detection Methods:', 10, 'bold');
      const monitoringStatus = categoryStatus.find(c => c.name === "Continuous Monitoring")?.completed;
      if (monitoringStatus) {
        addWrappedText('  - Automated Performance Monitoring: Real-time accuracy and bias metrics tracking', 10, 'normal');
        addWrappedText('  - Anomaly Detection: Statistical process control for model drift identification', 10, 'normal');
        addWrappedText('  - User Feedback Systems: Structured complaint and feedback collection mechanisms', 10, 'normal');
      } else {
        addWrappedText('  - DEVELOPMENT REQUIRED: Systematic monitoring framework to be implemented', 10, 'normal');
        addWrappedText('  - Priority: Establish baseline metrics and automated alerting systems', 10, 'normal');
      }
      
      addWrappedText('• Response Timeline:', 10, 'bold');
      addWrappedText('  - Critical Issues (Bias/Safety): Immediate response within 2 hours, resolution within 24 hours', 10, 'normal');
      addWrappedText('  - Performance Degradation: Assessment within 24 hours, corrective action within 72 hours', 10, 'normal');
      addWrappedText('  - Compliance Violations: Legal review within 4 hours, remediation plan within 48 hours', 10, 'normal');
      addWrappedText('  - Routine Issues: Standard triage process with 5-day resolution target', 10, 'normal');
        yPosition += 5;
      ...
      pdf.setFontSize(10);
      pdf.setFont('helvetica', 'normal');
      addWrappedText('Strategic priorities to enhance risk posture and regulatory readiness:', 10, 'normal');
      yPosition += 5;
      
      const strategicNextSteps = [
        `1. Priority Assessment Completion: Complete ${totalSections - completedSectionsCount} remaining assessment domains within 30 days to establish comprehensive risk baseline. Rationale: Eliminates regulatory blind spots and enables confident deployment in high-stakes financial environments.`,
      ...
      const reportProjectName = assessmentData.aiSystemDescription || 'AI System Assessment';
      const truncatedProjectName = reportProjectName.length > 50 ? reportProjectName.substring(0, 50) + '...' : reportProjectName;
      ...
      addWrappedText(`Framework Applied: NIST AI Risk Management Framework (AI RMF 1.0)`, 10, 'normal');
      addWrappedText(`Evaluator: AI Governance Assessment Platform`, 10, 'normal');
      addWrappedText(`Report ID: AGP-${Date.now().toString().slice(-8)}`, 10, 'normal');
  ...
  const analyzeQuestion = (field: string, question: string, answer: string, sectionContext: string): QuestionAnalysis => {
    let riskScore = 0;
    ...
    if (field === 'aiSystemDescription') {
      if (answerLower.includes('machine learning') || answerLower.includes('neural') || answerLower.includes('deep learning')) {
        riskScore += 15;
        riskFactors.push('Complex ML system requiring enhanced governance');
        recommendations.push('Implement ML model versioning and lineage tracking');
        recommendations.push('Establish comprehensive model validation procedures');
        regulatoryImpact.push('Subject to AI/ML regulatory scrutiny');
        businessImpact = 'High technical complexity requiring specialized expertise and governance frameworks';
        criticalityLevel = 'High';
      }
      if (answerLower.includes('automated decision') || answerLower.includes('autonomous')) {
        riskScore += 20;
        riskFactors.push('Automated decision-making system with potential human impact');
        recommendations.push('Implement human-in-the-loop mechanisms for critical decisions');
        recommendations.push('Establish clear decision audit trails and explainability features');
        regulatoryImpact.push('EU AI Act high-risk system classification potential');
        regulatoryImpact.push('GDPR Article 22 automated decision-making provisions');
        criticalityLevel = 'Critical';
      }
      if (answerLower.includes('financial') || answerLower.includes('credit') || answerLower.includes('fraud')) {
        riskScore += 25;
        riskFactors.push('Financial sector application with regulatory implications');
        recommendations.push('Ensure compliance with Dodd-Frank Act and Basel III requirements');
        recommendations.push('Implement comprehensive financial model risk management');
        regulatoryImpact.push('Federal Reserve SR 11-7 model risk management guidance');
        regulatoryImpact.push('OCC 2011-12 supervisory guidance on model risk management');
        businessImpact = 'Critical financial compliance requirements with potential regulatory penalties';
        criticalityLevel = 'Critical';
      }
      ...
      if (answer.length < 50) {
        ...
        recommendations.push('Provide comprehensive system architecture documentation');
        complianceLevel = 'Low';
      }
    }
    ...
    // Human and Stakeholder Involvement Analysis
      ...
      if (answerLower === 'no') {
        riskScore += 30;
        riskFactors.push('Lack of documented AI governance roles creates accountability gaps');
        recommendations.push('Establish RACI matrix with AI Ethics Officer, Data Steward, Model Validator roles');
        recommendations.push('Define clear escalation procedures and decision-making authority');
        regulatoryImpact.push('Non-compliance with corporate governance requirements');
        businessImpact = 'High operational risk due to unclear accountability and decision-making processes';
        complianceLevel = 'Non-Compliant';
        criticalityLevel = 'Critical';
      } else if (answerLower === 'yes') {
        riskScore += 5;
        recommendations.push('Enhance role documentation with quarterly reviews and competency assessments');
        recommendations.push('Implement role-based training and certification programs');
        businessImpact = 'Good governance foundation requiring ongoing maintenance and improvement';
      }
    }
      ...
      if (answerLower === 'no') {
        riskScore += 25;
        riskFactors.push('Untrained personnel pose significant operational and ethical risks');
        recommendations.push('Implement comprehensive AI ethics and bias detection training program');
        recommendations.push('Establish competency assessments and continuous education requirements');
        regulatoryImpact.push('Potential non-compliance with duty of care requirements');
        businessImpact = 'High risk of poor decision-making and ethical violations due to inadequate training';
        complianceLevel = 'Low';
        criticalityLevel = 'High';
      } else if (answerLower === 'yes') {
        recommendations.push('Track training effectiveness through performance metrics and feedback');
        recommendations.push('Update training curricula to reflect emerging AI governance practices');
        businessImpact = 'Positive investment in human capital reducing operational risks';
      }
    }
      ...
      if (answerLower === 'no') {
        riskScore += 35;
        riskFactors.push('Lack of human override capability creates safety and accountability risks');
        recommendations.push('Implement emergency stop and override mechanisms for all critical AI decisions');
        recommendations.push('Establish clear protocols for human intervention scenarios');
        regulatoryImpact.push('Non-compliance with EU AI Act human oversight requirements');
        regulatoryImpact.push('Potential GDPR Article 22 violations for automated decisions');
        businessImpact = 'Critical safety and liability risk requiring immediate remediation';
        complianceLevel = 'Non-Compliant';
        criticalityLevel = 'Critical';
      }
    }
    ...
    // Safety and Reliability Analysis
      ...
      if (answerLower === 'no') {
        riskScore += 40;
        riskFactors.push('Unidentified threats create significant security and operational vulnerabilities');
        recommendations.push('Conduct comprehensive threat modeling including adversarial attacks');
        recommendations.push('Implement systematic vulnerability assessments and penetration testing');
        regulatoryImpact.push('Non-compliance with cybersecurity frameworks and regulations');
        businessImpact = 'Severe security exposure with potential for data breaches and system compromise';
        complianceLevel = 'Non-Compliant';
        criticalityLevel = 'Critical';
      } else if (answerLower.includes('yes') || answerLower.includes('identified')) {
        riskScore += 10;
        recommendations.push('Regularly update threat assessments based on emerging attack vectors');
        recommendations.push('Implement continuous monitoring and threat intelligence integration');
        businessImpact = 'Proactive security posture requiring ongoing vigilance and updates';
      }
    }
      ...
      if (answerLower === 'no') {
        riskScore += 30;
        riskFactors.push('Unassessed malicious use potential creates dual-use and reputational risks');
        recommendations.push('Conduct comprehensive dual-use assessment and misuse scenario analysis');
        recommendations.push('Implement usage monitoring and anomaly detection systems');
        regulatoryImpact.push('Potential non-compliance with export control regulations');
        businessImpact = 'Reputational and legal risk from unintended harmful applications';
        complianceLevel = 'Low';
        criticalityLevel = 'High';
      }
    }
    ...
    // Privacy and Data Governance Analysis
      ...
      if (answerLower === 'yes') {
        riskScore += 20;
        riskFactors.push('Personal data processing triggers comprehensive privacy obligations');
        recommendations.push('Implement data minimization and purpose limitation principles');
        recommendations.push('Establish comprehensive consent management and data subject rights procedures');
        regulatoryImpact.push('GDPR, CCPA, and other privacy law compliance requirements');
        regulatoryImpact.push('Potential for significant privacy violations and penalties');
        businessImpact = 'Significant compliance obligations requiring dedicated privacy resources';
        criticalityLevel = 'High';
      }
    }
      ...
      if (answerLower === 'no') {
        riskScore += 25;
        riskFactors.push('Lack of privacy risk assessment creates compliance and breach vulnerabilities');
        recommendations.push('Conduct Data Protection Impact Assessment (DPIA) immediately');
        recommendations.push('Implement privacy by design principles throughout system architecture');
        regulatoryImpact.push('GDPR Article 35 DPIA requirement non-compliance');
        businessImpact = 'High privacy violation risk with potential for regulatory penalties';
        complianceLevel = 'Non-Compliant';
        criticalityLevel = 'High';
      }
    }
      ...
      if (answerLower === 'no') {
        riskScore += 20;
        riskFactors.push('Poor data quality undermines AI system reliability and fairness');
        recommendations.push('Implement statistical process control for data quality monitoring');
        recommendations.push('Establish data lineage tracking and automated quality checks');
        businessImpact = 'Degraded AI performance leading to poor business outcomes and user trust issues';
        criticalityLevel = 'High';
      }
    }
  ...
  const analyzeSectionOverall = (sectionTitle: string, questionAnalyses: QuestionAnalysis[]): SectionAnalysis => {
    const sectionNumber = sectionTitle.includes('AI System Information') ? 1 :
                         sectionTitle.includes('Human and Stakeholder') ? 2 :
                         sectionTitle.includes('Safety and Reliability') ? 4 :
                         sectionTitle.includes('Privacy and Data') ? 7 : 0;

    const totalRiskScore = questionAnalyses.reduce((sum, qa) => sum + qa.riskScore, 0);
    const avgRiskScore = questionAnalyses.length > 0 ? totalRiskScore / questionAnalyses.length : 0;
    ...
    const timelineRecommendations: { action: string; priority: string; timeline: string }[] = [];

    // Extract insights from question analyses
    questionAnalyses.forEach(qa => {
      if (qa.riskScore <= 10) {
        keyStrengths.push(`${qa.field}: Good compliance with ${qa.complianceLevel.toLowerCase()} risk level`);
      } else {
      ...
      qa.recommendations.forEach(rec => {
        if (qa.criticalityLevel === 'Critical') {
          ...
          timelineRecommendations.push({ action: rec, priority: 'Critical', timeline: 'Immediate (0-30 days)' });
        } else if (qa.criticalityLevel === 'High') {
          timelineRecommendations.push({ action: rec, priority: 'High', timeline: 'Short-term (1-3 months)' });
        } else {
          timelineRecommendations.push({ action: rec, priority: 'Medium', timeline: 'Medium-term (3-6 months)' });
        }
      });
    ...
    if (sectionTitle.includes('AI System Information')) {
      if (avgRiskScore > 15) {
        priorityActions.unshift('CRITICAL: Establish comprehensive AI system documentation and governance framework');
      }
      regulatoryAlignment.push('NIST AI Risk Management Framework alignment required');
    } else if (sectionTitle.includes('Human and Stakeholder')) {
      if (avgRiskScore > 20) {
        priorityActions.unshift('CRITICAL: Implement human oversight and governance mechanisms immediately');
      }
      regulatoryAlignment.push('EU AI Act human oversight requirements');
    } else if (sectionTitle.includes('Safety and Reliability')) {
      if (avgRiskScore > 25) {
        priorityActions.unshift('CRITICAL: Conduct immediate security assessment and implement safety controls');
      }
      regulatoryAlignment.push('ISO 31000 risk management standards');
    } else if (sectionTitle.includes('Privacy and Data')) {
      if (avgRiskScore > 15) {
        priorityActions.unshift('CRITICAL: Implement comprehensive privacy protection measures');
      }
